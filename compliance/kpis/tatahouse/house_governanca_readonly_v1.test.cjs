...
(async () => {
            ...
            data: [
              {
                ...
                restricoes_afetadas: ['NUNCA TRANSPORTAR'],
                dia_id: 'interno-1'
              },
              {
                data: '2026-08-24',
  ...
  assert.equal(JSON.stringify(ev).includes('NUNCA TRANSPORTAR'), false, 'comentários/restrições individuais não podem sair do Líderes');
  assert.equal(JSON.stringify(ev).includes('interno-1'), false, 'IDs internos não podem sair do Líderes');

  const d1 = ev.dias[0];
  assert.equal(d1.planejadoTotal, 83);
  assert.equal(d1.servidoTotal, 79);
  assert.equal(d1.almocoServido, 58);
  assert.equal(d1.indiceSaudavel, 82);
  assert.equal(d1.kcal, 710);
  assert.equal(d1.proteinaG, 38);
  ...
  assert.equal(frango.desperdicioMedioDia, 10);
  assert.equal(frango.servidoMedioDia, 81);
  assert.equal(frango.planejadoMedioDia, 83.5);
  ...
  assert.equal(api.normalizarEvidencia({ ...ev, dias: [{ ...ev.dias[0], servidoTotal: 999 }, ...ev.dias.slice(1)] }), null, 'payload adulterado precisa falhar fechado');
  ...
  assert.equal(post.msg.payload.dias[0].servidoTotal, 79);
  ...
  console.log('READONLY_OPERATIONAL_METRICS=PASS');
